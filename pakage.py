...
def read_json_package():
    try : 
        ...
        return FileNotFoundError("could not found package.json file")
...
def create_vsix(data , args):
    output_path = "./"
    targets = ['win32-x64','win32-arm64', 'linux-x64', 'linux-arm64',
    ...
    def ignore_function(directory, contents):  
        if args.target:
            ...
            targets.clear()
        items_to_exclude = ['extension.vsixmanifest', '[Content_Types].xml' , 'package-lock.json', 'node_modules' , 'vscode-drawio-1.6.6'] + targets
